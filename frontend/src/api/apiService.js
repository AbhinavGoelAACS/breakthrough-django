...
export const acsApi = {
  ...
  careers: {
    ...
    admin: {
      ...
      updateApplication: (applicationId, payload) =>
        apiService.patch(`/api/v1/admin/careers/applications/${applicationId}`, payload),
  ...
  author: {
    ...
    submitPaper: ({ title, abstract, keywords, journal_id, title_page, blinded_manuscript, research_area, paper_type, message_to_editor, paper_references, terms_accepted, authors }) => {
      console.log('submitPaper called with:', { 
        ...
        paper_type,
        terms_accepted,
        authors_count: authors?.length || 0
      });
      ...
      formData.append('paper_references', paper_references || '');
  ...
  editor: {
    ...
    cancelInvitation: (paperId, invitationId) =>
      apiService.delete(`/api/v1/editor/papers/${paperId}/invitations?invitation_id=${invitationId}`),
    remindInvitation: (paperId, invitationId) =>
      apiService.post(`/api/v1/editor/papers/${paperId}/invitations/${invitationId}/remind`),
