# Generated by Django 6.0.3 on 2026-03-12 20:14

from django.db import migrations, models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
    ]

    operations = [
        migrations.CreateModel(
            name='Editor',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('editor_name', models.CharField(blank=True, max_length=100, null=True)),
                ('editor_email', models.CharField(blank=True, max_length=100, null=True)),
                ('editor_address', models.CharField(blank=True, max_length=200, null=True)),
                ('editor_contact', models.CharField(blank=True, max_length=100, null=True)),
                ('editor_affiliation', models.CharField(blank=True, max_length=200, null=True)),
                ('editor_department', models.CharField(blank=True, max_length=200, null=True)),
                ('editor_college', models.CharField(blank=True, max_length=200, null=True)),
                ('password', models.CharField(blank=True, max_length=200, null=True)),
                ('journal_id', models.IntegerField(blank=True, null=True)),
                ('role', models.CharField(blank=True, max_length=50, null=True)),
                ('editor_type', models.CharField(blank=True, max_length=50, null=True)),
                ('added_on', models.DateTimeField(blank=True, null=True)),
            ],
            options={
                'db_table': 'editor',
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='EmailTemplate',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('name', models.CharField(max_length=100)),
                ('slug', models.CharField(max_length=50, unique=True)),
                ('subject', models.CharField(max_length=500)),
                ('body_template', models.TextField()),
                ('placeholders', models.TextField(blank=True, null=True)),
                ('category', models.CharField(max_length=50)),
                ('is_active', models.BooleanField(default=True)),
                ('created_at', models.DateTimeField()),
                ('updated_at', models.DateTimeField(blank=True, null=True)),
            ],
            options={
                'db_table': 'email_template',
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='Issue',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('pages', models.CharField(blank=True, max_length=7, null=True)),
                ('month', models.CharField(blank=True, max_length=16, null=True)),
                ('volume', models.IntegerField(blank=True, null=True)),
                ('journal', models.IntegerField(blank=True, null=True)),
                ('add_on', models.CharField(blank=True, max_length=10, null=True)),
                ('issue_no', models.IntegerField(blank=True, null=True)),
                ('complete_issue', models.CharField(blank=True, max_length=10, null=True)),
            ],
            options={
                'db_table': 'issue',
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='Journal',
            fields=[
                ('fld_id', models.AutoField(primary_key=True, serialize=False)),
                ('fld_journal_name', models.CharField(blank=True, max_length=200, null=True)),
                ('freq', models.CharField(blank=True, max_length=250, null=True)),
                ('issn_ol', models.CharField(blank=True, max_length=250, null=True)),
                ('issn_prt', models.CharField(blank=True, max_length=250, null=True)),
                ('cheif_editor', models.CharField(blank=True, max_length=250, null=True)),
                ('co_editor', models.CharField(blank=True, max_length=250, null=True)),
                ('password', models.CharField(max_length=100)),
                ('abs_ind', models.CharField(blank=True, max_length=300, null=True)),
                ('short_form', models.CharField(max_length=255)),
                ('journal_image', models.CharField(max_length=255)),
                ('journal_logo', models.CharField(max_length=200)),
                ('guidelines', models.CharField(max_length=200)),
                ('copyright', models.CharField(max_length=200)),
                ('membership', models.CharField(max_length=200)),
                ('subscription', models.CharField(max_length=200)),
                ('publication', models.CharField(max_length=200)),
                ('advertisement', models.CharField(max_length=200)),
                ('description', models.TextField()),
                ('added_on', models.DateField()),
            ],
            options={
                'db_table': 'journal',
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='JournalDetails',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('journal_id', models.CharField(max_length=50)),
                ('about_journal', models.TextField(blank=True, null=True)),
                ('cheif_say', models.TextField(blank=True, null=True)),
                ('aim_objective', models.TextField(blank=True, null=True)),
                ('criteria', models.TextField(blank=True, null=True)),
                ('scope', models.TextField(blank=True, null=True)),
                ('guidelines', models.TextField(blank=True, null=True)),
                ('readings', models.TextField(blank=True, null=True)),
                ('added_on', models.DateTimeField()),
            ],
            options={
                'db_table': 'journal_details',
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='News',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('title', models.CharField(max_length=300)),
                ('description', models.TextField(blank=True, null=True)),
                ('added_on', models.DateField(blank=True, null=True)),
                ('journal_id', models.IntegerField(blank=True, null=True)),
            ],
            options={
                'db_table': 'news',
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='OnlineReview',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('paper_id', models.IntegerField(blank=True, null=True)),
                ('reviewer_id', models.CharField(blank=True, max_length=100, null=True)),
                ('assigned_on', models.DateField(blank=True, null=True)),
                ('submitted_on', models.DateTimeField(blank=True, null=True)),
                ('date_submitted', models.DateTimeField(blank=True, null=True)),
                ('review_status', models.CharField(default='pending', max_length=50)),
                ('review_submission_id', models.IntegerField(blank=True, null=True)),
                ('invitation_id', models.IntegerField(blank=True, null=True)),
                ('due_date', models.DateTimeField(blank=True, null=True)),
            ],
            options={
                'db_table': 'online_review',
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='Paper',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('paper_code', models.CharField(blank=True, max_length=200)),
                ('journal', models.IntegerField(blank=True, null=True)),
                ('title', models.CharField(default='', max_length=500)),
                ('abstract', models.CharField(default='', max_length=2500)),
                ('keyword', models.CharField(default='', max_length=1000)),
                ('file', models.CharField(default='', max_length=200)),
                ('title_page', models.CharField(blank=True, default='', max_length=200, null=True)),
                ('blinded_manuscript', models.CharField(blank=True, default='', max_length=200, null=True)),
                ('revised_track_changes', models.CharField(blank=True, default='', max_length=200, null=True)),
                ('revised_clean', models.CharField(blank=True, default='', max_length=200, null=True)),
                ('response_to_reviewer', models.CharField(blank=True, default='', max_length=200, null=True)),
                ('added_on', models.DateTimeField(blank=True, null=True)),
                ('added_by', models.CharField(default='', max_length=100)),
                ('status', models.CharField(default='submitted', max_length=50)),
                ('mailstatus', models.CharField(default='0', max_length=10)),
                ('volume', models.CharField(default='', max_length=100)),
                ('issue', models.CharField(default='', max_length=100)),
                ('author', models.CharField(default='', max_length=100)),
                ('coauth', models.CharField(default='', max_length=200)),
                ('rev', models.CharField(default='', max_length=200)),
                ('version_number', models.IntegerField(default=1)),
                ('revision_count', models.IntegerField(default=0)),
                ('revision_deadline', models.DateTimeField(blank=True, null=True)),
                ('revision_notes', models.TextField(blank=True, null=True)),
                ('revision_requested_date', models.DateTimeField(blank=True, null=True)),
                ('revision_type', models.CharField(blank=True, max_length=20, null=True)),
                ('editor_comments', models.TextField(blank=True, null=True)),
                ('research_area', models.CharField(blank=True, max_length=200, null=True)),
                ('message_to_editor', models.TextField(blank=True, null=True)),
                ('terms_accepted', models.BooleanField(default=False)),
            ],
            options={
                'db_table': 'paper',
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='PaperCoAuthor',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('paper_id', models.IntegerField()),
                ('salutation', models.CharField(blank=True, max_length=20, null=True)),
                ('first_name', models.CharField(max_length=100)),
                ('middle_name', models.CharField(blank=True, max_length=100, null=True)),
                ('last_name', models.CharField(max_length=100)),
                ('email', models.CharField(blank=True, max_length=255, null=True)),
                ('designation', models.CharField(blank=True, max_length=100, null=True)),
                ('department', models.CharField(blank=True, max_length=200, null=True)),
                ('organisation', models.CharField(blank=True, max_length=255, null=True)),
                ('author_order', models.IntegerField(default=1)),
                ('is_corresponding', models.BooleanField(default=False)),
                ('created_at', models.DateTimeField(blank=True, null=True)),
            ],
            options={
                'db_table': 'paper_co_author',
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='PaperComment',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('paper_id', models.IntegerField(blank=True, null=True)),
                ('comment_by', models.CharField(blank=True, max_length=255, null=True)),
                ('comment_text', models.TextField(blank=True, null=True)),
                ('added_on', models.DateTimeField(blank=True, null=True)),
            ],
            options={
                'db_table': 'paper_comment',
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='PaperCorrespondence',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('paper_id', models.IntegerField()),
                ('sender_id', models.IntegerField(blank=True, null=True)),
                ('sender_role', models.CharField(blank=True, max_length=50, null=True)),
                ('recipient_email', models.CharField(max_length=255)),
                ('recipient_name', models.CharField(blank=True, max_length=255, null=True)),
                ('subject', models.CharField(max_length=500)),
                ('body', models.TextField()),
                ('template_id', models.IntegerField(blank=True, null=True)),
                ('email_type', models.CharField(max_length=50)),
                ('status_at_send', models.CharField(blank=True, max_length=50, null=True)),
                ('is_read', models.BooleanField(default=False)),
                ('read_at', models.DateTimeField(blank=True, null=True)),
                ('delivery_status', models.CharField(default='pending', max_length=50)),
                ('webhook_id', models.CharField(blank=True, max_length=100, null=True)),
                ('webhook_received_at', models.DateTimeField(blank=True, null=True)),
                ('error_message', models.TextField(blank=True, null=True)),
                ('retry_count', models.IntegerField(default=0)),
                ('created_at', models.DateTimeField(blank=True, null=True)),
                ('sent_at', models.DateTimeField(blank=True, null=True)),
            ],
            options={
                'db_table': 'paper_correspondence',
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='PaperPublished',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('title', models.CharField(max_length=250)),
                ('abstract', models.CharField(max_length=2000)),
                ('p_reference', models.TextField(blank=True, null=True)),
                ('author', models.CharField(max_length=1000)),
                ('journal', models.CharField(max_length=250)),
                ('journal_id', models.IntegerField()),
                ('volume', models.CharField(max_length=250)),
                ('issue', models.CharField(max_length=250)),
                ('date', models.DateTimeField()),
                ('pages', models.CharField(max_length=250)),
                ('keyword', models.CharField(max_length=300)),
                ('language', models.CharField(max_length=20)),
                ('paper', models.CharField(blank=True, max_length=200, null=True)),
                ('access_type', models.CharField(max_length=20)),
                ('email', models.CharField(blank=True, max_length=100, null=True)),
                ('affiliation', models.CharField(blank=True, max_length=500, null=True)),
                ('co_authors_json', models.TextField(blank=True, null=True)),
                ('doi', models.CharField(blank=True, max_length=100, null=True)),
                ('doi_status', models.CharField(max_length=50)),
                ('doi_registered_at', models.DateTimeField(blank=True, null=True)),
                ('crossref_batch_id', models.CharField(blank=True, max_length=100, null=True)),
                ('paper_submission_id', models.IntegerField(blank=True, null=True)),
            ],
            options={
                'db_table': 'paper_published',
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='PaperVersion',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('paper_id', models.IntegerField()),
                ('version_number', models.IntegerField()),
                ('file', models.CharField(max_length=200)),
                ('file_size', models.IntegerField(blank=True, null=True)),
                ('uploaded_on', models.DateTimeField(blank=True, null=True)),
                ('revision_reason', models.TextField(blank=True, null=True)),
                ('change_summary', models.TextField(blank=True, null=True)),
                ('uploaded_by', models.CharField(max_length=100)),
            ],
            options={
                'db_table': 'paper_version',
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='ReviewerInvitation',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('paper_id', models.IntegerField()),
                ('reviewer_id', models.IntegerField(blank=True, null=True)),
                ('reviewer_email', models.CharField(max_length=255)),
                ('reviewer_name', models.CharField(blank=True, max_length=255, null=True)),
                ('journal_id', models.CharField(blank=True, max_length=100, null=True)),
                ('invitation_token', models.CharField(max_length=255)),
                ('token_expiry', models.DateTimeField()),
                ('status', models.CharField(default='pending', max_length=50)),
                ('invited_on', models.DateTimeField(blank=True, null=True)),
                ('accepted_on', models.DateTimeField(blank=True, null=True)),
                ('declined_on', models.DateTimeField(blank=True, null=True)),
                ('invitation_message', models.TextField(blank=True, null=True)),
                ('decline_reason', models.TextField(blank=True, null=True)),
                ('is_external', models.BooleanField(default=False)),
            ],
            options={
                'db_table': 'reviewer_invitation',
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='ReviewSubmission',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('paper_id', models.IntegerField()),
                ('reviewer_id', models.CharField(max_length=100)),
                ('assignment_id', models.IntegerField(blank=True, null=True)),
                ('paper_version', models.IntegerField(default=1)),
                ('technical_quality', models.IntegerField(blank=True, null=True)),
                ('clarity', models.IntegerField(blank=True, null=True)),
                ('originality', models.IntegerField(blank=True, null=True)),
                ('significance', models.IntegerField(blank=True, null=True)),
                ('overall_rating', models.IntegerField(blank=True, null=True)),
                ('author_comments', models.TextField(blank=True, null=True)),
                ('confidential_comments', models.TextField(blank=True, null=True)),
                ('recommendation', models.CharField(blank=True, max_length=50, null=True)),
                ('review_report_file', models.CharField(blank=True, max_length=500, null=True)),
                ('file_version', models.IntegerField(default=1)),
                ('status', models.CharField(default='draft', max_length=50)),
                ('created_at', models.DateTimeField(blank=True, null=True)),
                ('updated_at', models.DateTimeField(blank=True, null=True)),
                ('submitted_at', models.DateTimeField(blank=True, null=True)),
            ],
            options={
                'db_table': 'review_submission',
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='RoleRequest',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('user_id', models.IntegerField()),
                ('requested_role', models.CharField(max_length=50)),
                ('status', models.CharField(default='pending', max_length=20)),
                ('reason', models.TextField(blank=True, null=True)),
                ('requested_at', models.DateTimeField()),
                ('processed_by', models.IntegerField(blank=True, null=True)),
                ('processed_at', models.DateTimeField(blank=True, null=True)),
                ('admin_notes', models.TextField(blank=True, null=True)),
            ],
            options={
                'db_table': 'role_request',
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='User',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('email', models.CharField(max_length=255, unique=True)),
                ('password', models.CharField(max_length=255)),
                ('role', models.CharField(blank=True, default='User', max_length=50, null=True)),
                ('fname', models.CharField(blank=True, max_length=100, null=True)),
                ('lname', models.CharField(blank=True, max_length=100, null=True)),
                ('mname', models.CharField(blank=True, max_length=100, null=True)),
                ('title', models.CharField(blank=True, max_length=100, null=True)),
                ('affiliation', models.CharField(blank=True, max_length=255, null=True)),
                ('specialization', models.TextField(blank=True, null=True)),
                ('contact', models.CharField(blank=True, max_length=20, null=True)),
                ('address', models.TextField(blank=True, null=True)),
                ('added_on', models.DateTimeField(blank=True, null=True)),
                ('salutation', models.CharField(blank=True, max_length=20, null=True)),
                ('designation', models.CharField(blank=True, max_length=100, null=True)),
                ('department', models.CharField(blank=True, max_length=200, null=True)),
                ('organisation', models.CharField(blank=True, max_length=255, null=True)),
            ],
            options={
                'db_table': 'user',
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='UserRole',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('user_id', models.IntegerField()),
                ('role', models.CharField(max_length=50)),
                ('status', models.CharField(max_length=20)),
                ('requested_at', models.DateTimeField()),
                ('approved_by', models.IntegerField(blank=True, null=True)),
                ('approved_at', models.DateTimeField(blank=True, null=True)),
                ('rejected_reason', models.TextField(blank=True, null=True)),
                ('journal_id', models.IntegerField(blank=True, null=True)),
                ('editor_type', models.CharField(blank=True, max_length=50, null=True)),
            ],
            options={
                'db_table': 'user_role',
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='Volume',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('journal', models.CharField(max_length=250)),
                ('volume_no', models.IntegerField()),
                ('year', models.CharField(blank=True, max_length=200, null=True)),
                ('added_on', models.DateField(blank=True, null=True)),
            ],
            options={
                'db_table': 'volume',
                'managed': False,
            },
        ),
    ]
